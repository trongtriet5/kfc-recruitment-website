...
export default function RecruitmentDashboard() {
  ...
  const hasFilters =
    campaignId !== 'ALL' || storeId !== 'ALL' || taId !== 'ALL' || dateRange.from || dateRange.to;
    ...
    return (
      <div className="pt-6 space-y-8 animate-pulse">
        ...
        <div className="flex gap-3 pb-6 border-b border-gray-100">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-10 bg-gray-200 rounded-md w-[220px]" />
          ))}
        </div>
        {/* KPI cards skeleton */}
  ...
  const acceptedCount =
    candidatesByStatus?.find((s: any) => s.statusCode === 'ONBOARDING_ACCEPTED')?.count || 0;

  const statusChartData = {
    series: candidatesByStatus.filter(s => s.count > 0).map(s => s.count),
    labels: candidatesByStatus.filter(s => s.count > 0).map(s => s.statusName),
    colors: candidatesByStatus.filter(s => s.count > 0).map(s => s.statusColor || '#6B7280'),
  };
